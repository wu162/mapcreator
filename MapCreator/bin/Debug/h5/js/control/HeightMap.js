...
class HeightMap {
    ...
    addTerrain() {
        ...
        this.mesh = new THREE.Mesh(this.geometry, new THREE.MeshLambertMaterial({map: this.textureMap.texture, side: THREE.DoubleSide}));
        this.app.scene.add(this.mesh);
    }

    onPointerMove(intersect) {
        if (this.leftDown && this.app.mode == 1) {
            for (let i = -this.app.radius;i <= this.app.radius;i++) {
                for (let j = -this.app.radius;j <= this.app.radius;j++) {
